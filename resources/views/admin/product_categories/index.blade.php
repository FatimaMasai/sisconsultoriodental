<x-admin-layout>
    <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between mb-6">
        <x-label class="text-black text-lg sm:text-xl font-semibold">
            Listado de Categoría de Producto
        </x-label>
        <div class="flex flex-wrap items-center gap-2">
            @can('admin.product_categories.pdf')
                <a href="{{ route('admin.product_categories.pdf') }}" class="btn btn-orange" target="_blank">
                    <i class="fa-solid fa-file-pdf mr-1"></i> PDF
                </a>
                <a href="{{ route('admin.product_categories.excel') }}" class="btn btn-green">
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-6">
        <form method="GET" action="{{ route('admin.product_categories.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                @if (request('search'))
                    <a href="{{ route('admin.product_categories.index') }}" class="btn btn-gray rounded-lg text-sm whitespace-nowrap">
                            ...
                            <td class="px-3 py-2">
                                <div class="flex space-x-2">
                                    @can('admin.product_categories.edit')
                                        <a href="{{ route('admin.product_categories.edit', $category) }}" class="btn btn-blue text-xs">Editar</a>
                                    @endcan

                                    @can('admin.product_categories.destroy')
                                        <form class="delete-form" action="{{ route('admin.product_categories.destroy', $category) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
    ...
    @else
        ...
        <div class="flex items-center p-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
            ...
            <div>
                ...
                @else
                    Todavia no hay categorías de producto registradas.
                @endif
            </div>
        </div>
