<div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-6">
        <div class="flex items-center gap-2 mb-4">
            ...
            <x-label class="text-black dark:text-white text-base font-semibold">Buscar en Categorías de Producto</x-label>
        </div>
        ...
        @if (trim($search) !== '')
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-3">
                {{ $product_categories->total() }} resultado(s) para "{{ $search }}"
            </p>
        @endif
    </div>
                    ...
                    @foreach ($product_categories as $category)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                            <th scope="row" data-label="ID" class="px-3 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ ($product_categories->currentPage() - 1) * $product_categories->perPage() + $loop->iteration }}
                            </th>
                            <td data-label="Nombre" class="px-3 py-2">
                                {{ $category->name }}
                            </td>
                            <td data-label="Fecha de registro" class="px-3 py-2">
