<div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-6">
        <div class="flex items-center gap-2 mb-4">
            ...
            <x-label class="text-black dark:text-white text-base font-semibold">Buscar en Proveedores</x-label>
        </div>
        ...
        @if (trim($search) !== '')
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-3">
                {{ $suppliers->total() }} resultado(s) para "{{ $search }}"
            </p>
        @endif
    </div>
                ...
                <tbody>
                    @foreach ($suppliers as $supplier)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                            ...
                            <td data-label="Empresa" class="px-3 py-2">
                                {{ $supplier->company }}
                            </td>
                            <td data-label="NIT" class="px-3 py-2">
                                {{ $supplier->nit }}
                            </td>
