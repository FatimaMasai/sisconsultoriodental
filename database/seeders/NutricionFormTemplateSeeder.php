 ...
 *
 * Es idempotente: si la especialidad ya tiene una plantilla activa CON
 * secciones, no hace nada (para no duplicar si se corre dos veces). Si la
 * plantilla existe pero todavía está vacía (por ejemplo, se creó desde la
 * pantalla del constructor pero no se le agregó nada), le carga las
 * secciones ahí mismo en vez de crear una plantilla nueva.
 *
 * Ejecutar con:
 *   php artisan db:seed --class=NutricionFormTemplateSeeder
 */
class NutricionFormTemplateSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($template && $template->sections()->exists()) {
            $this->command?->warn(
                'La especialidad "Nutrición" ya tiene una plantilla activa con secciones cargadas (id ' . $template->id . '). No se modificó nada.'
            );
        ...
        $this->command?->info('Plantilla de Nutrición cargada correctamente.');
    }
    ...
    private function secciones(): array
    {
        return [
            [
                ...
                'fields' => [
                    ['label' => 'Motivo de consulta', 'type' => 'texto', 'required' => true],
                ],
            ],
            [
